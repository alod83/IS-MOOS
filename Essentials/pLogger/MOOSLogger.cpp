...
bool CMOOSLogger::LogSystemMessages(MOOSMSG_LIST &NewMail)
{
	...
	string key = rMsg.GetKey();
        if(IsSystemMessage(key) && !rMsg.IsSkewed(dfTimeNow))
        {

            m_SystemLogFile<<setw(10)<<setprecision(7)<<rMsg.m_dfTime-GetAppStartTime()<<' ';

            m_SystemLogFile<<setw(20)<<rMsg.GetKey().c_str()<<' ';

            m_SystemLogFile<<setw(20)<<rMsg.GetSource()<<' ';
...
bool CMOOSLogger::DoAsyncLog(MOOSMSG_LIST &NewMail)
{
    ...
    if(m_bAsynchronousLog)
    {
        ...
        for(q = NewMail.begin();q!=NewMail.end();q++)
        {
            ...
            if(m_MOOSVars.find(rMsg.GetKey())!=m_MOOSVars.end())
            {
				
				int i=0;
				if(m_bUseExcludedLog)
				{
					switch(GetDestinationLog(rMsg.GetKey()))
					{
						case XLOG: i = 1; break;
						case ALOG: i = 0; break;
						default:
