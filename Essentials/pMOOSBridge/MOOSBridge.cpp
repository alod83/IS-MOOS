                ...
                if(pDestCommunity!=pSrcCommunity)
                {
                    ...
                    for(w = InMail.begin();w!=InMail.end();w++)
                    {
                        ...
                        {    
                            ...
                            CMOOSMsg MsgCopy = *w;
                            MsgCopy.SetKey(pDestCommunity->GetAlias(Index));
                            if(IsUDPShare(Index))
                            {
                                ...
                                {
                                    MOOSTrace("cannot send %c via UDP to %s - destination community has no UDP port\n",MsgCopy.m_sKey,pDestCommunity->GetFormattedName().c_str());
                                }
                            }
                            else
                            {
	...
	if(m_UDPLink.Fetch(UDPMail))
    {
        ...
        if(qq!=m_Communities.end())
        {
            ...
            for(p = UDPMail.begin();p!=UDPMail.end();p++)
            {
                ...
                if(!pLocalCommunity->HasMOOSSRegistration(p->GetKey()))

// && pLocalCommunity->GetCommunityName()!= p->GetCommunity() )
                {
