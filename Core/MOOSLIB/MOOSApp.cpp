...
CMOOSApp::CMOOSApp()
{
    ...
    m_dCommsFreq=DEFAULT_MOOS_APP_COMMS_FREQ;
    m_nIterateCount = 0;
    m_nMailCount = 0;
    m_bServerSet = false;
    m_dfAppStartTime = -1;
    ...
    if(sMissionFile!=NULL)
    {
        ...
        if(1)
        {
            ...
            //m_MissionReader.GetConfigurationParam("APPTICK",m_dfFreq);
             m_MissionReader.GetValue("APPTICK",m_dfFreq);
...
bool CMOOSApp::ConfigureComms()
{
    ...
    m_MissionReader.GetValue("COMMSTICK",m_dCommsFreq);
    m_dCommsFreq = m_dCommsFreq <0 ? 1 : m_dCommsFreq;
    ...
    int iUDPReadTime = 8;
    m_MissionReader.GetValue("UDP_WAIT_TIME",iUDPReadTime);

    int iReadTimeout = 5;
    if(!m_MissionReader.GetValue("READ_TIMEOUT",iReadTimeout))
    {
	MOOSTrace("%sWARNING: Missing Read Timeout - Assuming '%d'%s\n", red,iReadTimeout,none); 
    }
    ...
    if(m_sMOOSName.empty())
        m_sMOOSName = m_sAppName;
    m_Comms.InitSecurity(m_MissionReader);
    
    //m_Comms.Run(m_sServerHost.c_str(),m_lServerPort,m_sMOOSName.c_str(),m_dCommsFreq);
    m_Comms.Run(m_sServerHost.c_str(),m_lServerPort,m_sMOOSName.c_str(),m_sTransportProtocol,m_dCommsFreq,iReadTimeout,iUDPReadTime);
...
bool CMOOSApp::SetCommsFreq(unsigned int nFreq)
{
    ...
    {
        m_dCommsFreq = nFreq;
        return m_Comms.SetCommsTick(m_dCommsFreq);
    }
    return false;
}
