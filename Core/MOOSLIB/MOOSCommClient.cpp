...
CMOOSCommClient::CMOOSCommClient()
{
	...
	m_bConnected = false;
	m_dFundamentalFreq = CLIENT_DEFAULT_FUNDAMENTAL_FREQ;
	m_nNextMsgID=0;
	m_bFakeSource = false;
    m_bQuiet= false;
...
bool CMOOSCommClient::Run(const char *sServer, long lPort, const char * sMyName, std::string transportProtocol, double dFundamentalFrequency,int iReadTimeout, short int iUDPWaitTime)
{
	m_bQuit = false;
	...
	m_dFundamentalFreq=dFundamentalFrequency;

	if(m_dFundamentalFreq>CLIENT_MAX_FUNDAMENTAL_FREQ)
	{
		MOOSTrace("Setting Fundamental Freq to maximum value of %d Hz\n",CLIENT_MAX_FUNDAMENTAL_FREQ);
		m_dFundamentalFreq=CLIENT_MAX_FUNDAMENTAL_FREQ;
	}
	else
	{
...
bool CMOOSCommClient::SetCommsTick(double dCommTick)
{
    if(dCommTick>CLIENT_MAX_FUNDAMENTAL_FREQ)
	{
		MOOSTrace("Setting Fundamental Freq to maximum value of %d Hz\n",CLIENT_MAX_FUNDAMENTAL_FREQ);
		m_dFundamentalFreq=CLIENT_MAX_FUNDAMENTAL_FREQ;
        return false;
    ...
    else
    {
        ...
        m_dFundamentalFreq = dCommTick;
        if(m_dFundamentalFreq==0)//catch a stupid setting
            m_dFundamentalFreq = 1;
        return true;
    }
...
bool CMOOSCommClient::ClientLoop()
{
	...
	while(!m_bQuit)
	{
		...
		if(m_sTransportProtocol == "TCP")
			...
			m_pSocket = new XPCUdpSocket(m_lPort,m_iReadTimeout, m_iUDPWaitTime);
		if(ConnectToServer())
		{
			...
			MOOSTrace("COMMSCLIENT DEBUG: Tick period %f ms (should be %d ms)\n",MOOSLocalTime()-dfTDebug,(int)(1000.0/m_dFundamentalFreq));
                ...
                if(m_dFundamentalFreq==0)
                    m_dFundamentalFreq=1;
                
				MOOSPause((int)(1000.0/m_dFundamentalFreq));
...
bool CMOOSCommClient::HandShake()
{
	...
	try
	{
		...
		//char mName = m_sMyName[0];
		//MOOSTrace("name '%c'\n", mName);
		CMOOSMsg Msg(MOOS_DATA,"",m_sMyName);
		unsigned char tNonce[sizeof(int)];
		RAND_bytes(tNonce, sizeof(int));
		Msg.m_nID = atoi((const char*)tNonce);
		int nonceC = Msg.m_nID;
		SendMsg(m_pSocket,Msg, ek);
		
		if(m_oSecurity.isAuthenticationEnabled())
		{
			CMOOSMsg KMsg;
			m_oSecurity.setSessionKey(m_sMyName,0);
			if(ReadMsg(m_pSocket,KMsg,-1,ek))
			{
				if(nonceC == KMsg.m_nID)
				{
					sKey = (unsigned char*) malloc(EVP_MAX_KEY_LENGTH);
					memcpy(sKey,(const char*) KMsg.m_sVal.c_str(),EVP_MAX_KEY_LENGTH);

					MOOSTrace("%sSuccessfully Got session key%s\n",green,none);

	 				Msg.m_dfTime = KMsg.m_dfTime;
	 				m_oSecurity.setSessionKey(m_sMyName,sKey);
					SendMsg(m_pSocket,Msg);
				}
				else
				{
					MOOSTrace("%sWARNING: Authentication failed - bad nonce\n%s",red,none);	
					return false;
				}
			}
			else
			{
				MOOSTrace("%sWARNING: Authentication failed - failed to read session key\n%s",red,none);
				return false;
			}
		}
		...
		if(!ReadMsg(m_pSocket,WelcomeMsg))
 		{
 			MOOSTrace("Failed to read welcome message\n");
 			return false;
 		}
		...
		else
		{
			...
			double dfSkew = WelcomeMsg.GetDouble();
			//double dfSkew = WelcomeMsg. m_dfTime;
            		if(m_bDoLocalTimeCorrection)
			    	SetMOOSSkew(dfSkew);
...
void CMOOSCommClient::DoBanner()
{
	...
	if(m_sTransportProtocol == "UDP")
		MOOSTrace("*       %sUDP Wait Time %s'%d'%s                \n",magenta,blue, m_iUDPWaitTime,none);
...
unsigned char* CMOOSCommClient::getSessionKey(XPCSocket *pSocket)
{
	return (unsigned char*) m_oSecurity.getSessionKey(m_sMyName);
}
