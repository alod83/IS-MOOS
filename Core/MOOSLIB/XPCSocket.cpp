...
XPCSocket::XPCSocket(const char *_sProtocol, int _iPort)
{
    ...
    try
    {
	    ...
	    int optval = 1;
   	    ...
   	    if(setsockopt(iSocket, SOL_SOCKET, SO_KEEPALIVE, &optval, optlen) < 0)
 	    {
                char sMsg[512];

                sprintf(sMsg, "Error while setting keepalive options: %s", sGetError());
                XPCException socketExcept(sMsg);
                throw socketExcept;
                return;
            }
...
int XPCSocket::iSendMessage(void *_vMessage, int _iMessageSize){}

    // Receives a message 
int XPCSocket::iRecieveMessage(void *_vMessage, int _iMessageSize, int _iOption){}

    // Binds the socket to an address and port number
void XPCSocket::vBindSocket(){}

    // Accepts a connecting client.  The address of the connected client is stored in the parameter
XPCSocket *XPCSocket::Accept(char *_sHost){}

    // Listens to connecting clients
void XPCSocket::vListen(int _iNumPorts){}

    // Connects to a client specified by a supplied host name
void XPCSocket::vConnect(const char *_sHost){}

    // allows a read with a timeout to prevent from blocking indefinitely
int XPCSocket::iReadMessageWithTimeOut(void *_vMessage, int _iMessageSize, double dfTimeOut,int _iOption){}
