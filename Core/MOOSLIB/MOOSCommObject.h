...
protected:
    bool SendPkt(XPCSocket* pSocket,CMOOSCommPkt & PktTx, unsigned char* key = 0);
    bool ReadPkt(XPCSocket* pSocket,CMOOSCommPkt & PktRx,int nSecondsTimeOut = -1, unsigned char* key = 0);
    bool SendMsg(XPCSocket* pSocket,CMOOSMsg & Msg, unsigned char* key = 0);
    bool ReadMsg(XPCSocket* pSocket,CMOOSMsg & Msg, int nSecondsTimeOut = -1,unsigned char* key = 0);
    
    
    /// called to intialise system socket services. Only does something useful in Win32 land
public:
    CMOOSSecurity m_oSecurity;
    static bool SocketsInit();
    void InitSecurity(CMOOSFileReader& m_MissionReader);
    void DoSecurityBanner();
