...
bool UdpThreadLoopProc( void * pParameter)
{
    struct param* p = (struct param*) pParameter;  
    CMOOSCommServer* pMe =     p->cs;
    XPCSocket* s = p->s;

    return pMe->UdpThreadLoop(s);
}
...
bool CMOOSCommServer::Run(long lPort,std::string transportProtocol,bool bDisableNameLookUp, int num_ports,int min_port,int iReadTimeout)
{

    //m_sCommunityName = sCommunityName;
    m_sTransportProtocol = transportProtocol;
    m_lListenPort = lPort;
    m_iNumPorts = num_ports;
    m_iMinPort = min_port;
...
bool  CMOOSCommServer::OnAbsentClient(XPCSocket* pClient)
{
    MOOSTrace("\n------------ABSENT CLIENT---------\n");
    ...
    m_Socket2UdpThreadMap[pClient->iGetSocketFd()].Stop();
    m_Socket2UdpThreadMap.erase(pClient->iGetSocketFd());	
...
bool CMOOSCommServer::ListenLoop()
{
    ...
    if(m_sTransportProtocol == "TCP")
        ...
        m_pListenSocket = new XPCUdpSocket(m_lListenPort,m_iNumPorts,m_iMinPort,m_iReadTimeout);
    try
    {
    ...
    while(1)
    {
		...
		if(m_sTransportProtocol == "UDP")
		{
			CMOOSThread* mt = new CMOOSThread();
			m_Socket2UdpThreadMap[pNewSocket->iGetSocketFd()] = *mt;
			struct param p;
			p.cs = this;
			p.s = pNewSocket;
			mt->Initialise(UdpThreadLoopProc, &p);
			mt->Start();
		}
            }
...
bool CMOOSCommServer::UdpThreadLoop(XPCSocket* cSocket)
{
	   //optionally inhibit all MOOSTrace..    
    	if(m_bQuiet)
        	InhibitMOOSTraceInThisThread(true);
	...
	while(true)
	{
		try
		{
			...
			//read input
			ReadPkt(cSocket,PktRx);
			//convert to list of messages
			PktRx.Serialize(MsgLstRx,false);
		
			std::string sWho = m_Socket2ClientMap[cSocket->iGetSocketFd()];
			...
			if(!(*m_pfnRxCallBack)(sWho,MsgLstRx,MsgLstTx,m_pRxCallBackParam))
			{
				...
				MOOSTrace(" CMOOSCommServer::UdpThreadLoop()  pfnCallback failed\n");
			}
			...
			if( 1 || MsgLstTx.size()==0)
			{
				...
				CMOOSMsg NullMsg;
				NullMsg.SetDouble(MOOSLocalTime());
...
bool CMOOSCommServer::OnNewClient(XPCSocket * pNewClient,char * sName)
{
    MOOSTrace("\n------------CONNECT-------------\n");
...
bool CMOOSCommServer::OnClientDisconnect()
{
    ...
    if(p!=m_Socket2ClientMap.end())
    {
        ...
        MOOSTrace("%sClient \"%s\" has disconnected.%s\n",red,p->second.c_str(),none);
    ...
    if(m_pfnDisconnectCallBack!=NULL)
    {
        MOOSTrace("%sInvoking user OnDisconnect callback...%s\n",red,none);
		if(m_bQuiet)
			InhibitMOOSTraceInThisThread(false);
...
bool CMOOSCommServer::HandShake(XPCSocket *pNewClient)
{
    CMOOSMsg Msg;
    ...
    double nonceS;
    string cName;
    unsigned char* ek = (unsigned char*)m_oSecurity.getEncryptionKey();
    try
    {

	if(ReadMsg(pNewClient,Msg,m_iReadTimeout,ek))
        { 
		...
		cName = Msg.m_sVal;
		if(IsUniqueName(cName))
		{
			m_Socket2ClientMap[pNewClient->iGetSocketFd()] = cName;
		}
		else
		{
			PoisonClient(pNewClient,MOOSFormat("A client of this name (\"%s\") already exists", cName.c_str()));
			return false;
		}	
	}
	else
	...
	{
		unsigned char* sKey = (unsigned char*) malloc(EVP_MAX_KEY_LENGTH);
		m_oSecurity.setSessionKey(cName, 0);
		sKey = (unsigned char*)m_oSecurity.genSessionKey(cName);

		MOOSTrace("%sGenerated Session key for client %s'%s'%s\n",green,brown, cName.c_str(),none);
		
		string key((const char*) sKey);
		CMOOSMsg MsgK(MOOS_WELCOME,"",key);
		MsgK.m_nID = Msg.m_nID;
		
		unsigned char tNonce[sizeof(double)];
		RAND_bytes(tNonce, sizeof(double));
		Msg.m_dfTime = atof((const char*)tNonce);
		int nonceS = Msg.m_dfTime;
		double epsilon = 0.02;		

		SendMsg(pNewClient, MsgK, ek);
		m_oSecurity.setSessionKey(cName, sKey);

		if(ReadMsg(pNewClient,MsgK,m_iReadTimeout))
		{
			if(nonceS - MsgK.m_dfTime < epsilon)
				MOOSTrace("%sClient %s'%s'%s successfully authenticated%s\n", green,brown,cName.c_str(),green,none);
			else
			{
				MOOSTrace("%sWARNING: Client authentication failed - bad nonce%s\n", red,none);
				return false;
			}
		}
		else
		{
			MOOSTrace("%sClient authentication failed - failed to read key confirmation%s\n", red,none);
			return false;
		}
	}
...
void CMOOSCommServer::PoisonClient(XPCSocket *pSocket, const std::string & sReason)
{
    //kill the client...
...
string CMOOSCommServer::GetClientName(XPCSocket *pSocket)
{
    SOCKETFD_2_CLIENT_NAME_MAP::iterator p;
...
void CMOOSCommServer::DoBanner()
{
    ...
    MOOSTrace("*       %sMOOS VERSION %s '1.8'%s\n", magenta, blue,none); 
    MOOSTrace("*       %sTransport protocol %s'%s'%s\n", magenta, blue,m_sTransportProtocol.c_str(),none);
    MOOSTrace("*       %sRead Timeout %s'%d'%s\n",magenta,blue,m_iReadTimeout,none);
...
unsigned char* CMOOSCommServer::getSessionKey(XPCSocket *pSocket)
{
	return (unsigned char*)m_oSecurity.getSessionKey(m_Socket2ClientMap[pSocket->iGetSocketFd()]);
}
