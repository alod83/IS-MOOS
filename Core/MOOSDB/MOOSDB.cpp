...
//     }
    ...
    if(!m_MissionReader.GetValue("TRANSPORT_PROTOCOL",m_sTransportProtocol))
    {
	m_sTransportProtocol = "TCP"; 
        MOOSTrace("%sWARNING: Missing transport protocol - Assuming TCP%s\n",red,none); 
    }
    ...
    {
	if(!m_MissionReader.GetValue("NUM_PORTS",num_ports))
		MOOSTrace("%sWARNING: Missing Num_ports - Assuming NUM_PORTS = '%d'%s\n",red,num_ports,none);

	if(!m_MissionReader.GetValue("MIN_PORT",min_port))
		MOOSTrace("%sWARNING: Missing Min_port - Assuming MIN_PORT = '%d'%s\n",red,min_port,none);

    }
    
    int iReadTimeout = 5;
    if(!m_MissionReader.GetValue("READ_TIMEOUT",iReadTimeout))
	MOOSTrace("%sWARNING: Missing Read_timeout - Assuming READ_TIMEOUT = '%d'%s\n",red,iReadTimeout,none);
    ...
    LogStartTime();
    m_CommServer.InitSecurity(m_MissionReader);
    m_CommServer.Run(m_nPort,m_sTransportProtocol,bNoNetwork,num_ports,min_port,iReadTimeout);
...
//         
        ...
        {
        case MOOS_DOUBLE:
            rVar.m_dfVal = Msg.GetDouble();
	    rVar.m_cDataType = MOOS_DOUBLE;
            break;
        default:
            rVar.m_sVal = Msg.m_sVal;
            break;
        ...
        rVar.m_Writers.insert(Msg.GetSource());
...
/*    else
    {
        MOOSTrace("Attempting to update var \"%s\" which is type %c \n",
            Msg.m_sKey.c_str(),
            Msg.m_cDataType);
    }*/
...
bool CMOOSDB::OnUnRegister(CMOOSMsg &Msg)
{
    ...
    //to the client
    bool bAlreadyThere = VariableExists(Msg.GetKey());
    if(bAlreadyThere)
    {
        CMOOSDBVar & rVar  = GetOrMakeVar(Msg);
	string src(Msg.GetSource());
	rVar.RemoveSubscriber(src);
    }
...
bool CMOOSDB::OnRegister(CMOOSMsg &Msg)
{
    ...
    if(!rVar.AddSubscriber(src,Msg.GetDouble()))
        return false;
    ...
    if(bAlreadyThere && rVar.m_nWrittenTo!=0)
    {
        ...
        AddMessageToClientBox(Msg.GetSource(),ReplyMsg);      
...
{    
    ...
    DBVAR_MAP::iterator p = m_VarMap.find(Msg.GetKey());
    ...
    if(p==m_VarMap.end())
    {
        ...
        CMOOSDBVar NewVar(Msg.GetKey());
        
        //index our new creation
        m_VarMap[Msg.GetKey()] = NewVar;
        
        //check we can get it back ok!!
        p = m_VarMap.find(Msg.GetKey());
...
bool CMOOSDB::DoServerRequest(CMOOSMsg &Msg, MOOSMSG_LIST &MsgTxList)
{
    ...
    else if(Msg.GetKey().find("PROC_SUMMARY")!=string::npos)
    {
        return OnProcessSummaryRequested(Msg,MsgTxList);
    }
    else if (Msg.GetKey().find("VAR_SUMMARY") != string::npos) 
    {
        return OnVarSummaryRequested(Msg, MsgTxList);
    }
    else if(Msg.GetKey().find("DB_CLEAR")!=string::npos)
    {
...
bool CMOOSDB::OnProcessSummaryRequested(CMOOSMsg &Msg, MOOSMSG_LIST &MsgTxList)
{
    ...
    for(q=Clients.begin();q!=Clients.end();q++)
    {
        ...
        MsgReply.SetSource(m_sDBName);
        MsgReply.SetKey("PROC_SUMMARY");
...
bool CMOOSDB::OnServerAllRequested(CMOOSMsg &Msg, MOOSMSG_LIST &MsgTxList)
{
    ...
    for(p=m_VarMap.begin();p!=m_VarMap.end();p++)
    {
        ...
        MsgVar.SetSource(rVar.m_sWhoChangedMe);
        MsgVar.SetKey(rVar.m_sName);
...
bool CMOOSDB::OnVarSummaryRequested(CMOOSMsg &Msg, MOOSMSG_LIST &MsgTxList)
{
    ...
    Reply.SetSource(m_sDBName);
    Reply.SetKey("VAR_SUMMARY");
