...
**/
bool CSimpleApp::OnNewMail(MOOSMSG_LIST &list)
{
...
MOOSMSG_LIST::iterator p;
	for(p = list.begin(); p != list.end(); p++) {
		CMOOSMsg &Msg = *p;
		MOOSTrace("Got Message '%s' timestamp '%d'\n", Msg.m_sVal.c_str(), Msg.m_dfTime);
	}
...
{ 
	...
	m_Comms.Notify("VehicleStatus","ok",MOOSTime());
...
bool CSimpleApp::OnHeading(CMOOSMsg & Msg)
{
   ...
   */ double dfTime = Msg.GetTime();
    MOOSTrace("The heading (according to process %s),at time %f (%f since appstart) is \n",
